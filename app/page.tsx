...
import { analyzeImageAndGenerateRecipe, type ApiResponse, type RecipeResponse } from './actions'
import { ErrorBoundary } from './components/ErrorBoundary'
import { useSession } from 'next-auth/react'
import { signIn } from 'next-auth/react'
import { Button } from '@/components/ui/button'
import { UserMenu } from './components/UserMenu'
import { exampleRecipes } from './data/example-recipes'

function isRecipeResponse(response: ApiResponse): response is RecipeResponse {
...
export default function Home() {
  ...
  const handleExampleSelect = (index: number) => {
    ...
    setTimeout(() => {
      const result = exampleRecipes[index]
      if (!result) {
        setError('Failed to get example recipe')
        setIsLoading(false)
        return
      }
        ...
        <footer className="mt-24 pb-8 text-center text-sm text-gray-500 border-t pt-8">
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-center gap-4">
              <a
                href="https://github.com/Mistral-AI-quickstarts/pic2cook"
                target="_blank"
                rel="noopener noreferrer"
